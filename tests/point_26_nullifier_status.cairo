...
fn deploy_nullifier_status_stack() -> ISemaphoreDispatcher {
...
#[test]
fn point_26_nullifier_status_transitions_after_successful_validation() {
    let semaphore = deploy_nullifier_status_stack();
    let proof = build_submission_proof();

    assert(!semaphore.is_nullifier_used(NULLIFIER), 'POINT26_NULL_PRE');
    ...
    assert(semaphore.is_nullifier_used(NULLIFIER), 'POINT26_NULL_POST');
}
